...
import { list, reset } from '../../actions/newsarticle/list';
import { success } from '../../actions/newsarticle/delete';
      ...
      <p><Link to="create" className="btn btn-primary">Create</Link></p>
        ...
        {this.props.data['hydra:member'] && this.props.data['hydra:member'].map(item =>
          <tr key={item['@id']}>
            ...
            <td>{item['body'] ? itemToLinks(item['body']) : ''}</td>
...
const mapStateToProps = (state) => {
  return {
    data: state.newsarticle.list.data,
    error: state.newsarticle.list.error,
    loading: state.newsarticle.list.loading,
    deletedItem: state.newsarticle.del.deleted,
  };
};
